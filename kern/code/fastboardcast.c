...
#include "fastboardcast.skel.h"
#include "fastboardcast.h"

#define LO_IFINDEX 2 // modify it
...
{	
    ...
    printf("Handle=%u", tc_opts.handle);
	printf("Priority=%u", tc_opts.priority);

	bool hook_created = false;
	struct fastboardcast_bpf *skel;
	int err;
	int index, prog_fd;
	...
	skel = fastboardcast_bpf__open_and_load();
	if (!skel) {
...
cleanup:
	...
	fastboardcast_bpf__destroy(skel);
	return -err;
}

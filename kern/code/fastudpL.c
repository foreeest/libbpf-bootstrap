...
void convert_mac_to_hex(const char* mac_str, unsigned char* eth) {
    int values[ETH_ALEN];
    if (sscanf(mac_str, "%x:%x:%x:%x:%x:%x", 
               &values[0], &values[1], &values[2], 
               &values[3], &values[4], &values[5]) == ETH_ALEN) {
        for (int i = 0; i < ETH_ALEN; ++i) {
            eth[i] = (unsigned char) values[i];
			// printf("eth[i] is %d\n",eth[i]);
        }
    } else {
...
{	
	...
	skel = fastudpL_bpf__open_and_load();

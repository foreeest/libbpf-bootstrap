...
/* program maps */
// 7个map 2个.map
// 2个progs，1个cfg，3个pin的ringbuf和ctr_state，1个lastOp；1个共享bitset，1个batch相关的
// maps和.maps区别是？
// 这两个map_progs存放程序相关信息：存了程序的fd，调用的时候要给程序的idx
// 参考链接：https://ebpf-docs.dylanreimerink.nl/linux/map-type/BPF_MAP_TYPE_PROG_ARRAY/
// https://ebpf-go.dev/concepts/object-lifecycle/#program-arrays
...
// 	.max_entries = FAST_PROG_XDP_MAX,// 这宏在哪？在common.h，为5，为何我vscode没有高亮这个宏
// };
...
struct {
	__uint(type, BPF_MAP_TYPE_ARRAY);
	__type(key, __u32);// 0513 : u32 --> __u32
...
// 这个函数就是 xdp_dispatcher
/* 详情见手册https://ebpf-docs.dylanreimerink.nl/linux/program-type/BPF_PROG_TYPE_XDP/#device-map-program
或者中文讲解https://cloud.tencent.com/developer/article/1626925
struct xdp_md {
	__u32 data;
	__u32 data_end;
	__u32 data_meta;
...
// §5的handleACK函数，拦截输入信息,Leader的wait-on-quorums，Drop掉大部分信息，保留达到QUORUM的
// 实现 xdp_ebpf ,对应论文Listing 1伪代码
// 这个函数raft应该大部分相同
SEC("xdp")
...
int WriteBuffer_main(struct xdp_md *ctx) {
	...
	if (payload + MAX_DATA_LEN < data_end) return XDP_PASS;//居然是 < ???

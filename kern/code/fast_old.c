...
void parse_cmdline(int argc, char *argv[]) {
	...
	snprintf(filename, sizeof(filename), ".output/%s.bpf.o", argv[0]);// 生成一个文件名fast_kern.o
...
void create_object() {
	...
	for (int i = 0; i < prog_count; i++) {
        ...
        err = bpf_program__set_log_level(progs[i].prog, LIBBPF_WARN);
        if (err) {
...
retry:		// LIBBPF_API int bpf_program__pin(struct bpf_program *prog, const char *path);
...
int main(int argc, char *argv[]) {
	...
	for (int i = 0; i < interface_count; i++) {// 按照Electrode的指令，这里其实就link了一个主函数xdp-handler
		...
		if (bpf_xdp_attach (interfaces_idx[i], xdp_main_prog_fd, xdp_flags, xdp_opts) < 0) {
	...
	for (int i = 0; i < interface_count; i++) {
		...
		xdp_opts->old_prog_fd = 0;
		bpf_xdp_detach(interfaces_idx[i], xdp_flags, xdp_opts);
	}
	for (int i = 0; i < interface_count && optind < argc; i++) {
